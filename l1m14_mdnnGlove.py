...
def build_model(lr = 0.0, lr_d = 0.0, units = 0, dr = 0.0):
    ...
    emb = SpatialDropout1D(dr)(x)

    x = Bidirectional(GRU(40, return_sequences = True, recurrent_dropout=0.1))(emb)
    x = Conv1D(60, kernel_size = 3, padding = "valid", activation='relu', strides=1)(x)
    avg_pool = GlobalAveragePooling1D()(x)
    max_pool = GlobalMaxPooling1D()(x)
    
    x = Bidirectional(GRU(60, return_sequences = True, recurrent_dropout=0.1))(emb)
    x = Conv1D(120, kernel_size = 2, padding = "valid", activation='relu', strides=1)(x)
    avg_pool1 = GlobalAveragePooling1D()(x)
    max_pool1 = GlobalMaxPooling1D()(x)
    
    x = concatenate([avg_pool, max_pool, avg_pool1, max_pool1])

    x = Dense(6, activation = "sigmoid")(x)
    model = Model(inputs = inp, outputs = x)
    model.compile(loss = "binary_crossentropy", optimizer = 'adam', metrics = ["accuracy"])
#    model.compile(loss = "binary_crossentropy", optimizer = Adam(lr = lr, decay = lr_d), metrics = ["accuracy"])
    History = model.fit(X_train, Y_train, batch_size = 32, epochs = 24, validation_data = (X_valid, Y_valid), 
                        verbose = 1, callbacks = [callback, early_stop])
    return model, History

model, History = build_model(lr = 1e-3, lr_d = 0, dr = 0.2)
