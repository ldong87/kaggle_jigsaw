...
from keras.layers import GlobalAveragePooling1D, GlobalMaxPooling1D, concatenate, SpatialDropout1D, Reshape
...
with open(path+'embed_sent2vec.pkl', 'r') as f:
    trn_vec, tst_vec = pk.load(f)
...
X_train, X_valid, Y_train, Y_valid = trn_vec[ind_trn,:], trn_vec[ind_val,:], y[ind_trn], y[ind_val]
...
def build_model():
    ...
    inp = Input(batch_shape=(None,max_len, 1))
#    emb_comment = Embedding(max_features, embed_size, weights=[embedding_matrix], trainable=train_embed)(inp)
#    emb_comment = SpatialDropout1D(spatial_dropout)(emb_comment)
    
    block1 = Conv1D(filter_nr, kernel_size=filter_size, padding='same', activation='linear')(inp)
    ...
    resize_emb = Conv1D(filter_nr, kernel_size=1, padding='same', activation='linear')(inp)
